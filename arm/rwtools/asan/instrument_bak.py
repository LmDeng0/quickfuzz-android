...
class Instrument():
    ...
    def instrument_mem_accesses(self):
        fpath = "/root/quickfuzz-liming2/arm/rwtools/asan/Wrong.txt"
        lines = []
        ...
        for line in lines:
            if "byte" in line:
                jump_no_instrument.append(line)
        print(jump_no_instrument)
        print("this is first one")
        print(jump_no_instrument[0])
        search_list = []
        search_list_large_offset = []
        for i in jump_no_instrument:
            for j in range(len(i)):
                if i[j] == 'L' and i[j+1] == 'C':
                    search_list.append('0x'+ i[j + 2: j + 7])
        print(search_list)
        for j in range(int(len(search_list)/2)-1):
            if search_list[2 * j + 1] not in search_list_large_offset:
                search_list_large_offset.append(search_list[2 * j])
                search_list_large_offset.append(search_list[2 * j + 1])
            else:
                for k in range(int(len(search_list_large_offset)/2)):
                    if int(search_list_large_offset[2*k + 1], 16) == int(search_list[2 * j + 1], 16):
                        if int(search_list_large_offset[2 * k], 16) < int(search_list[2 * j], 16):
                            search_list_large_offset[2*k] = search_list[2 * j]
                        break
        """ search_list_no_repat = []
        for i in range(int(len(search_list)/2)):
            if search_list[2*i] not in search_list_no_repat:
                search_list_no_repat.append(search_list[2*i])
                search_list_no_repat.append(search_list[2*i+1]) """
        #search_list = self.no_instrument_charge()
        bb_start_list = []
        bb_end_list = []
        for _, fn in self.rewriter.container.functions.items():
            ...
            ### if addr in rang()，set instrumnet_skip
            #if  addr
            #break
            ...
            for e, addr in enumerate(sorted(fn.bbstarts)):
                ...
                for i in range(int(len(search_list)/2)-1):
                    if int(search_list[2*i + 1], 16) <= bb_start <= int(search_list[2*i], 16):
                        set_no_instrument = 1
                        bb_start_list.append(bb_start)
                        bb_end_list.append(bb_end)
                        break
                for i in range(int(len(search_list)/2)-1):
                    if int(search_list[2*i + 1], 16) <= bb_end <= int(search_list[2*i], 16):
                        set_no_instrument = 1
                        bb_start_list.append(bb_start)
                        bb_end_list.append(bb_end)
                        break
