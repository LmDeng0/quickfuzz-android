...
class Symbolizer():
    ...
    def symbolize_cf_transfer(self, container, context=None):
        for _, function in container.functions.items():
            ...
            for inst_idx, instruction in enumerate(function.cache):
                ...
                is_call = instruction.cs.mnemonic in ["bl", "blr","br"]
    ...
    """ def symbolize_switch_tables(self, container, context):
        ...
        if not rodata:
            assert False
        address = 0x1671C               ###txt中第五段数据
        cases_list = []
        cases = 6                       ###txt中第二段数据
        jmptbl_addr = 0X0D220           ###txt中第一段数据
        #shift = expr.left.right.right
        size = 1                        ###txt中第四段数据
        base_case = 0x16720             ###txt中第六段数据
        value = [7,9,0,0,0,11]          ###txt中第三段数据，数据长度是第二段数据，size
        for i in range(cases):
            #value = self._memory_read(container, jmptbl_addr + i*size, size, signed=True)
            debug("VALUE:" + str(value[i]))
            addr = base_case + value[i]*(4)
            swlbl = "(.LC%x-.LC%x)/%d" % (addr, base_case, 4)
            memory_replace(container, jmptbl_addr + i*size, size, swlbl)
            cases_list += [addr]

        #function.add_switch(Jumptable(address, jmptbl_addr, size, base_case, cases_list)) """

    def symbolize_switch_tables_new(self, container=None, context=None, jmptbl_addr_list=[], case_list=[],
                                    value_list=[], size_list=[], address_list=[], base_case_list=[]):
        rodata = container.sections.get(".rodata", None)
        if not rodata:
            assert False
        for i in range(len(jmptbl_addr_list)):
            cases_list = []
            jmptbl_addr = jmptbl_addr_list[i]
            address = address_list[i]
            cases = case_list[i]
            size = size_list[i]
            base_case = base_case_list[i]
            value = value_list[i]
            ...
            # # shift = expr.left.right.right
            # size = 1  ###txt中第四段数据
            # base_case = 0x16720  ###address + 4
            # value = [7, 9, 0, 0, 0, 11]  ###txt中第三段数据，数据长度是第二段数据，size
            for i in range(cases):
                # value = self._memory_read(container, jmptbl_addr + i*size, size, signed=True)
                print("VALUE:" + str(value[i]))
                addr = base_case + value[i] * (4)
                swlbl = "(.LC%x-.LC%x)/%d" % (addr, base_case, 4)
                print(container, jmptbl_addr + i * size, size, swlbl)
                memory_replace(container, jmptbl_addr + i * size, size, swlbl)
                cases_list += [addr]

            # function.add_switch(Jumptable(address, jmptbl_addr, size, base_case, cases_list))

    def source_info_format(self, container, context, fpath="jump_table.txt"):
        lines = []
        ...
        # 行6： 追加， address + 4
        format_lines = []
        jmptbl_addr_list = [i for i in lines[0]]
        case_list = lines[1]
        value_list = []
        for i in range(len(case_list)):
            if i == 0:
                value_list.append(lines[2][0: case_list[i]])
            else:
    ...
    def _adjust_adrp_section_pointer(self, container, secname, orig_off, instruction):
        ...
        if secname == ".text":  # special case as this is the only non-data section we need
            ...
            base = container.sections[secname].base
        if secname.lower()  == ".bss":
            with open("bss_instruction_addr.txt", "a") as f:
                f.write(str( instruction.address) + "\n")
        reg_name = instruction.reg_writes()[0]
        diff = base - orig_off
        op = '-' if diff > 0 else '+'
        # add code here
