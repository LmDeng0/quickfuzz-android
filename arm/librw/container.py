...
class Container():
    ...
    def add_globals(self, globals):
        ...
        bss_doc = {}
        #### if found == bss ，保存相关的一些信息。location 和 gobj 里的函数 和 size
        for location, gobjs in globals.items():
            ...
            if found.lower() in [".bss"]:
                bss_doc[location] = gobjs
        ...
        fname = "%s_%s.txt" % ("bss", time.strftime("%Y%m%d%H%M%S", time.localtime()))
        with open("bss_func.txt","w") as f:
            json.dump(bss_doc,f)
