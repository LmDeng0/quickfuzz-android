...
const int MIN_MAX_FREQ_DIFF = MAX_FREQ - MIN_FREQ + 1;
...
//extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  rewind(fp);
  int length = fread(data, 1, size, fp);
  if(length < size) {
    printf("LENGTH << FILE SIZE");
  }
  fclose(fp);
  ...
  if (!validFreq(input_freq)) {
    // sampling frequencies must be reasonable
    srand(time(NULL));
    input_freq = rand() % MIN_MAX_FREQ_DIFF + MIN_FREQ;  
  }

  if (!validFreq(output_freq)) {
    // sampling frequencies must be reasonable
    srand(time(NULL) + 1);
    output_freq = rand() % MIN_MAX_FREQ_DIFF + MIN_FREQ;  
  }

  if (input_channels < 1 ||
      input_channels > (quality < AudioResampler::DYN_LOW_QUALITY ? 2 : 8)) {
    if (quality < AudioResampler::DYN_LOW_QUALITY) {
      srand(time(NULL));
      input_channels = rand() % 2 + 1;
    } else {
      srand(time(NULL));
      input_channels = rand() % 8 + 1;
    }
  }
  ...
  printf("AudioResampler::create");
  resampler->setSampleRate(input_freq);
  printf("AudioResampler::setSampleRate");
  resampler->setVolume(left_volume, right_volume);
  printf("AudioResampler::setVolume");
  ...
  for (size_t i = 0; i < output_frames; i++) {
    ...
    printf("AudioResampler::resample");
  }
